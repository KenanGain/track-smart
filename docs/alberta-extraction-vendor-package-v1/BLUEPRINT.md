# Blueprint — Alberta Carrier Profile vendor package (v1)
...
## Per-PDF specs shipped with v1

| PDF | Pages | Carrier | Profile period | v1 status |
|---|---:|---|---|---|
| `Carry_Freight_19_Dec_2018.pdf`     | 30 | Carry Freight Ltd. (AB243-8992) | 2016 DEC 20 → 2018 DEC 19 | ✅ Full per-pdf folder (canonical sample). Matches existing AB frontend mock data. |
| `Carrier_Profile_30_Sept_2019.pdf`  | 39 | Carry Freight Ltd. (AB243-8992) | 2017 OCT 01 → 2019 SEP 30 | Raw only. Same older format. **v2** target. |
| `Carrier_Profile_4Jan2021.pdf`      | 87 | (newer format)                  | TBD                       | Raw only. **Newer "Page X of Y" layout** — schema/parser extension required. **v2+** target. |
| `Carrier_Profile_07_Sep_2022.pdf`   | 33 | (newer format)                  | TBD                       | Raw only. **v2+** target. |
| `Carrier_Profile_21_Feb_2024.pdf`   | 46 | (newer format)                  | TBD                       | Raw only. **v2+** target. |
| `Carrier_Profile_15_Oct_2024.pdf`   | 64 | (newer format)                  | TBD                       | Raw only. **v2+** target. |
| `Carrier_Profile_6_Mar_2026.pdf`    | 43 | (newer format)                  | TBD                       | Raw only. **v2+** target. |

### Why v1 only ships one canonical example

The 2018 PDF is matched 1:1 against the existing TrackSmart AB frontend mock data, so the schema's correctness can be cross-validated against the rendered UI before broadening to other carriers. Once vendors / reviewers ack the schema, v2 will:

1. Process the 2019 PDF (same format) for cross-pull consistency on the same carrier.
2. Extend the schema with whatever the 2021+ "Page X of Y" Carrier Profile layout adds (it appears to drop some fields and add others).
3. Ship per-pdf folders for the remaining 5 newer-format PDFs.
