# Alberta Carrier Profile PDF Extraction — Vendor Package (v1)
...
**Status:** v1 — initial release. Ships the schema, validator, helper scripts, and **one fully-built canonical example** (`Carry_Freight_19_Dec_2018`). Six additional raw PDFs are included in `raw-pdfs/` and will be expanded into per-pdf folders in v2. Same organisation as the CVOR (Ontario) vendor package v6.
...
└── per-pdf/                 ← one folder per processed PDF (v1 ships the canonical only)
    └── Carry_Freight_19_Dec_2018/
```

**v1 scope**: The 6 newer-format Carrier Profiles in `raw-pdfs/` (2021, 2022, 2024 ×2, 2026) use Alberta's revised "Page X of Y" layout, which differs structurally from the older 2018-2019 format. v1 establishes the schema and methodology against the older format. v2 will broaden the schema and parser to cover the newer layout and ship per-pdf folders for all seven samples.
