# BC CVSE Carrier Profile Report PDF Extraction — Vendor Package (v6)
...
**Status:** v6 — contravention completeness release. v5 truthed §3 fleet, §5 CVSA, §6 accidents, §8 CVIP, §4.4 pending-carrier from the PDF. v6 closes the last big stale arrays: **§4.1 driver guilty 19 → 28**, **§4.2 carrier guilty 2 → 1**, **§4.3 pending driver 15 → 9** — all re-extracted from §4 pages 8-14 by an extended `scripts/rebuild_arrays_from_pdf.py` (now handles violation blocks with multi-line locations/descriptions, driver context carried across multiple violations, and optional Disposition Date / Active Points cells). **5866** highlights (up from 5767). Schema unchanged from v5. Validator stays green. Frontend code unchanged.
