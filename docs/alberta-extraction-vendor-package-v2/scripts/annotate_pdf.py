...
def hl_all(page, queries: Iterable, color, opacity: float = 0.45,
           filter_short: bool = False) -> int:
    n = 0
    for q in queries:
        if q is None:
            continue
...
def annotate(src_pdf: Path, dst: Path, extracted_path: Path | None = None) -> None:
    ...
    # Two passes per page: paint LABELS first (PALE), then VALUES on top (STRONG)
    # so the strong-colour values "win" visually wherever they overlap a label.
    for i in range(doc.page_count):
        ...
        if "Profile Period Start:" in text and "Date Printed:" in text and "Requested By:" in text:
            counts["YELLOW"] += hl_all(page, COVER_LABELS_YELLOW, PALE["YELLOW"])
            counts["GREEN"]  += hl_all(page, ["NSC Number:", "Carrier Name", "CARRIER INFORMATION"], PALE["GREEN"])
        if "TABLE OF CONTENTS" in text:
            counts["YELLOW"] += hl_all(page, COVER_LABELS_YELLOW, PALE["YELLOW"])
            counts["GREEN"]  += hl_all(page, ["NSC Number:", "Carrier Name:"], PALE["GREEN"])
        if "PART 1 - CARRIER INFORMATION" in text:
            counts["GREEN"]  += hl_all(page, PART1_LABELS_GREEN, PALE["GREEN"])
            counts["BLUE"]   += hl_all(page, PART1_LABELS_BLUE, PALE["BLUE"])
            counts["YELLOW"] += hl_all(page, ["12-month Report as of:"], PALE["YELLOW"])
        if "PART 2 - CONVICTION" in text or "CONVICTION NOTE" in text:
            counts["ORANGE"] += hl_all(page, PART2_LABELS, PALE["ORANGE"])
        ...
        if "PART 3 - CVSA INSPECTION" in text or "CVSA INSPECTION NOTE" in text:
            counts["PURPLE"] += hl_all(page, PART3_LABELS, PALE["PURPLE"])
            if "PART 3 - CVSA INSPECTION ANALYSIS" in text:
                counts["PURPLE"] += hl_all(page, CVSA_DEFECT_LABELS, PALE["PURPLE"])
        if "PART 4 - COLLISION" in text or "COLLISION NOTE" in text:
            counts["RED"]    += hl_all(page, PART4_LABELS, PALE["RED"])
        if "PART 5 - VIOLATION" in text or "VIOLATION NOTE" in text:
            counts["YELLOW"] += hl_all(page, PART5_LABELS, PALE["YELLOW"])
        ...
        if "PART 6 - MONITORING" in text or "MONITORING NOTE" in text:
            counts["BROWN"]  += hl_all(page, PART6_LABELS, PALE["BROWN"])

        # ── PASS 2: STRONG values (filtered: drop short/ambiguous tokens) ──
        if "Profile Period Start:" in text and "Date Printed:" in text and "Requested By:" in text:
            counts["YELLOW"] += hl_all(page, values["YELLOW"], STRONG["YELLOW"], filter_short=True)
            counts["GREEN"]  += hl_all(page, values["GREEN"],  STRONG["GREEN"],  filter_short=True)
        if "TABLE OF CONTENTS" in text:
            counts["GREEN"] += hl_all(page, values["GREEN"], STRONG["GREEN"], filter_short=True)
        if "PART 1 - CARRIER INFORMATION" in text:
            counts["GREEN"]  += hl_all(page, values["GREEN"],  STRONG["GREEN"],  filter_short=True)
            counts["BLUE"]   += hl_all(page, values["BLUE"],   STRONG["BLUE"],   filter_short=True)
            counts["YELLOW"] += hl_all(page, values["YELLOW"], STRONG["YELLOW"], filter_short=True)
        if "PART 2 - CONVICTION" in text or "CONVICTION NOTE" in text:
            counts["ORANGE"] += hl_all(page, values["ORANGE"], STRONG["ORANGE"], filter_short=True)
        if "PART 3 - CVSA INSPECTION" in text or "CVSA INSPECTION NOTE" in text:
            counts["PURPLE"] += hl_all(page, values["PURPLE"], STRONG["PURPLE"], filter_short=True)
        if "PART 4 - COLLISION" in text or "COLLISION NOTE" in text:
            counts["RED"]    += hl_all(page, values["RED"],    STRONG["RED"],    filter_short=True)
        if "PART 5 - VIOLATION" in text or "VIOLATION NOTE" in text:
            counts["YELLOW"] += hl_all(page, values["YELLOW"], STRONG["YELLOW"], filter_short=True)
        if "PART 6 - MONITORING" in text or "MONITORING NOTE" in text:
            counts["BROWN"]  += hl_all(page, values["BROWN"],  STRONG["BROWN"],  filter_short=True)
