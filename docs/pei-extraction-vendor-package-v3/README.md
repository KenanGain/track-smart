# PEI Carrier Abstract Report PDF Extraction — Vendor Package (v3)
...
**Status:** v3 — annotator now distinguishes **PALE labels from STRONG values** within each text band (mirrors the AB / CVOR overlay style). Per-pdf README cross-links `extraction-doc.md`. Carries forward v2's OCR reference pipeline (`scripts/extract_ocr.py`), v2's per-pdf walkthroughs, and v1's auto-detection annotator + 90 px column-merge gap. Schema, validator, hand-transcribed `extracted.json` files unchanged.
