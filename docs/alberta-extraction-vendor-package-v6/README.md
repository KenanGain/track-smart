# Alberta Carrier Profile PDF Extraction — Vendor Package (v6)
...
**Status:** v6 — full coverage release. Two canonical examples (`Carry_Freight_19_Dec_2018` and `Carrier_Profile_30_Sept_2019`). The annotated PDFs are a **complete parser-side visual map** — every label that anchors a value in the PDF is highlighted PALE in its sink colour, every value is highlighted STRONG. Rect-overlap dedup uses intersection-over-min-area so each text region gets exactly one annotation without falsely dropping line-adjacent labels. Schema includes additions for the newer 2021+ Carrier Profile layout (Certificate fields + new Part 3 Administrative Penalty Information).
