...
CVSA_DEFECT_LABELS = [
    ...
    "7 - Frames",
    "8 - Fuel Systems",
    # Cat 9 wording shifts between revisions of the form. Both variants are
    # listed so the annotator hits whichever the PDF prints.
    "9 - Lighting Devices (Part II Section 8 OOSC only)",
    "9 - Lighting Devices (Part II Section 9 only)",
    "9 - Lighting Devices",
    "10 - Cargo Securement",
    ...
    # Cat 17 is sometimes truncated to "(Buse" due to PDF column-width clipping.
    "17 - Emergency Exits/Wiring & Electrical Systems (Buses)",
    "17 - Emergency Exits/Wiring & Electrical Systems (Buse",
    "17 - Emergency Exits/Wiring & Electrical Systems",
    "18 - Dangerous Goods",
    "19 - Driveline/Driveshaft",
    # Newer-form category 20 (added 2019+).
    "20 - Driver's Seat (Missing)",
]

# Comprehensive label coverage — every label the vendor's PARSER must
# recognise to find a value, plus every section banner / column header /
# detail-block field name that anchors a region. The rect-overlap dedup in
# Highlighter.flush() ensures we never paint the same text region twice,
# so we can list freely here without producing muddy stacking.

COVER_LABELS_YELLOW = [
    "CARRIER PROFILE", "NOTE",
    "NSC Number:", "Carrier Name:",
...
PART1_LABELS_BLUE = [
    ...
    "Carrier's Monitoring Stage",
    "Total number of carriers at the same stage or greater:",
...
# Section banner labels — every Part repeats these on its first page.
SECTION_BANNER = [
...
PART2_LABELS = SECTION_BANNER + [
    ...
    # Banner totals
    "DOCUMENTS:", "CONVICTIONS:", "ACTIVE POINTS:",
    # Note-page point legend
    ...
    # Detail block headers + field labels — all named in every conviction detail.
    # NOTE: "VEHICLE:" appears TWICE per detail block: once at parent level and
    # once inside the numbered offence sub-block. Same label, both painted.
    "TIME", "JURISDICTION", "DATE ENTERED",
    "ISSUING AGENCY:", "LOCATION:", "DRIVER:", "VEHICLE:", "COMMODITY:",
    "CCMTA CODE:", "CONV DATE:", "DOCKET NO:",
    # End markers
    "*** END OF PART 2 ***", "TOTAL PAGES:",
]

PART3_LABELS = SECTION_BANNER + [
    ...
    # Banner totals
    "PASSED:", "REQUIRED ATTENTION:", "OUT OF SERVICE:",
    # Analysis table headers — full set of column words on the analysis page:
    #   NUMBER OF DEFECTS
    #   OUT OF    REQUIRES     TOTAL   PERCENT
    #   SERVICE   ATTENTION   DEFECTS  OF TOTAL DEFECT CATEGORY / DESCRIPTION
    "NUMBER OF DEFECTS", "OUT OF", "REQUIRES",
    "SERVICE", "ATTENTION", "DEFECTS",
    "TOTAL", "PERCENT", "OF TOTAL",
    "DEFECT CATEGORY / DESCRIPTION", "GRAND TOTAL DEFECTS",
    # CVSA Inspection SUMMARY column headers (page 'PART 3 - CVSA INSPECTION SUMMARY'):
    #   DATE   CVSA DOCUMENT   JUR   AGENCY   PLATE   LEVEL   RESULT
    "DATE", "CVSA", "DOCUMENT", "JUR", "AGENCY", "PLATE", "LEVEL", "RESULT",
    # CVSA Inspection DETAIL column headers (per-record top row):
    #   DATE   TIME   DOCUMENT   JUR   LEVEL   RESULT   DATE ENTERED
    "TIME", "DATE ENTERED",
    # CVSA Inspection DETAIL — the per-record vehicle sub-table columns:
    #   VEHICLE: TYPE  PLATE   JUR                 VIN   YEAR  MAKE   CVSA DECAL #
    "VEHICLE: TYPE", "TYPE", "VIN", "YEAR", "MAKE", "CVSA DECAL #",
    # CVSA Inspection DETAIL — agency/location/driver labels:
    "AGENCY:", "LOCATION:", "DRIVER:",
    # CVSA Inspection DETAIL — defect sub-table headers:
    "BY VEHICLE",
...
PART4_LABELS = SECTION_BANNER + [
    ...
    "NON-", "PREVENTABLE",
    "PREVENTABLE OR", "NOT EVALUATED",
    "ACTIVE", "POINTS",
    "Property Damage:", "Injury:", "Fatal:",
    # Note-page legend
    "Property Damage", "Personal Injury", "Fatality",
    "2 points", "4 points", "6 points",
    # Summary/detail column headers + labels
    "DATE", "DOCUMENT", "JUR", "PLATE", "STATUS", "SEVERITY",
    "ASSESSMENT:", "DRIVER:", "LOCATION:", "VEHICLE:",
    # Empty-state strings
    "No Collisions on Record for period selected",
    "*** END OF PART 4 ***",
]

PART5_LABELS = SECTION_BANNER + [
    "VIOLATION NOTE",
    "PART 5 - VIOLATION INFORMATION",
    ...
    # Summary table column headers
    "DATE", "DOCUMENT", "JUR", "VEHICLE", "DRIVER NAME",
    # Detail block field labels — Part 5 violation detail also names the
    # vehicle on the parent block AND inside each numbered offence.
    "TIME", "JURISDICTION", "DATE ENTERED",
    "ISSUING AGENCY:", "LOCATION:", "DRIVER:", "VEHICLE:", "COMMODITY:",
    "ACT/SECTION:", "CCMTA CODE:", "TEXT:",
    # Empty-state strings
    "No Violations on Record for period selected",
    "*** END OF PART 5 ***",
]

PART6_LABELS = SECTION_BANNER + [
    "MONITORING NOTE",
    "PART 6 - MONITORING INFORMATION",
    "PART 6 - MONITORING SUMMARY",
    "PART 6 - MONITORING DETAILS",
    # Monitoring summary column headers
    ...
    "and higher",
    # Monitoring details column headers
    "FLEET", "SIZE",
    "CONVICTIONS", "PTS/VEH",
    "CVSA INSPECTIONS",
...
class Highlighter:
    ...
    def _overlaps(self, a, b) -> bool:
        # Two PDF text rects can share a few pixels of y-range because of font
        # ascender/descender margins between adjacent lines (e.g. 'DRIVER:' on
        # one line and 'VEHICLE:' immediately below it). Counting that as
        # "overlapping" wrongly drops the second label.
        #
        # Real overlap is when the intersection area is a *substantial* fraction
        # of the smaller rect — i.e. one rect is mostly nested inside the other.
