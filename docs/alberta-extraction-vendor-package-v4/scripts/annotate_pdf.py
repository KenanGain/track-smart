...
# Label lists are intentionally narrow: only labels the AB frontend RENDERS
# as on-screen text. PDF column headers ("DATE", "DOCUMENT", "JUR", "PLATE")
# and detail-block field labels ("ISSUING AGENCY:", "LOCATION:", "DRIVER:",
# "COMMODITY:", "CCMTA CODE:", "CONV DATE:", "DOCKET NO:") are NOT highlighted —
# the frontend has its own column headers and inline labels for those fields,
# so highlighting their PDF equivalents adds noise without helping a vendor.

COVER_LABELS_YELLOW = [
    # The cover and TOC are mostly used to anchor the vendor on profile period
    # + date printed (which the frontend renders in the section banner) and to
    # confirm the report's part list (rendered as a side nav on the AB views).
...
PART1_LABELS_GREEN = [
    # The AB Carrier Profile sticky shows these field names verbatim.
    "PART 1 - CARRIER INFORMATION",
...
PART4_LABELS = [
    ...
    # Empty-state strings the vendor needs to recognise.
...
PART6_LABELS = [
    ...
    # The Industry Information block + Stage thresholds are both panels in the
    # frontend Monitoring Information Panel.
