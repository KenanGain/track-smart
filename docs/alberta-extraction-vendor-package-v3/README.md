# Alberta Carrier Profile PDF Extraction — Vendor Package (v2)
...
**Status:** v2 — second release. Ships the schema, validator, helper scripts, and **two fully-built canonical examples** (`Carry_Freight_19_Dec_2018` and `Carrier_Profile_30_Sept_2019`) covering the older Carrier Profile format. The annotator is now data-driven: it reads `extracted.json` and highlights every value automatically. Five 2021+ PDFs are included as raw samples; their newer "Page X of Y" layout adds a brand-new **Part 3 — Administrative Penalty Information** section and renumbers the remaining Parts, so they're scoped for v3.
