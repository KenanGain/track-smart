...
Coverage-everywhere release. **All 12 unique Alberta Carrier Profile PDFs from `docs/Alberta docs/`** now have a per-pdf folder.

**Added — 10 new per-pdf folders** (auto-bootstrapped to baseline depth)

| PDF | Format | Notes |
|---|---|---|
| `Carrier_Profile_09_Jan_2020`     | older                   | |
| `Carrier_Profile_4Jan2021`        | older→newer transition  | |
| `Carrier_Profile_03_May_2022`     | older                   | |
| `Carrier_Profile_07_Sep_2022`     | newer "Page X of Y"     | Same carrier as canonical (Carry Freight, AB243-8992) |
| `Carrier_Profile_17_May_2023`     | newer                   | |
| `Carrier_Profile_21_Feb_2024`     | newer                   | |
| `Carrier_Profile_15_Oct_2024`     | newer                   | |
| `Carrier_Profile_09_Oct_2025`     | newer                   | |
| `Carrier_Profile_6_Mar_2026`      | newer                   | |
| `Carrier_Profile_VM_MOTORS`       | varies                  | Different carrier (VM Motors) |

Each folder contains the same five deliverables as the v6 canonical samples:
- `extracted.json` — schema-valid, validator passes
- `annotated.pdf` — full 7-color overlay (data-driven values from extracted.json + comprehensive PALE labels)
- `lists/*.csv` — 16 flattened CSVs
- `README.md` — auto-generated summary card

**Added — `scripts/bootstrap_extracted.py`**
Detects PDF format (older multi-column vs newer "Page X of Y") and emits a baseline schema-valid `extracted.json` containing:
- carrier identity (NSC, MVID, address, safety fitness, operating status, fleet range/type, certificate fields when present)
- profile period + date printed
- pull-level metrics (R-Factor, contributions, fleet sizes, total carriers in Alberta)
- monitoring industry block (asOf, fleetRange, fleetType, avgRFactor)
- monitoring stage thresholds (parsed from PDF when printed; defaults otherwise)
- empty event lists across the board (totals=0) — full per-event extraction is the **v8** target

**Added — `scripts/write_readme.py`**
Auto-generates per-pdf README cards from extracted.json. Used to populate the 10 new folders' READMEs in one pass.

**Validator fix**
`Σ` characters in `validate.py` error messages were causing `UnicodeEncodeError` on Windows (cp1252). Replaced with ASCII `Sum` so the validator runs cleanly across platforms.

**Result**
`12/12 OK` on `python validate.py`. Every PDF in the source folder is now in the package, schema-valid, with annotated overlay and CSVs.

## Roadmap (v8)

For each of the 10 baseline samples, hand-extract the full event lists:
- Conviction summary + details (date, document, jurisdiction, vehicle, driver, offence, points, location, CCMTA code, conv date, docket, active points)
- CVSA inspection summary + details (date, time, document, jurisdiction, level, result, agency, location, driver, vehicles list with type/plate/jur/VIN/year/make, OOS + REQ defects with vehicle index)
- Collision summary + details (date, document, jurisdiction, plate, driver, severity, assessment, location, vehicle, VIN, points)
- Violation summary + details (date, document, jurisdiction, plate, driver, offences with Act/Section, CCMTA code/label, text)
- Monitoring summary + details (24-month rolling table)
- Per-section banner totals (DOCUMENTS:, CONVICTIONS:, CVSA inspection counts, OFFENCES:)
- New-format Part 3 (Administrative Penalty Information) for the 2022+ samples — admin penalty analysis (16 groups), summary, details with Act/Section + CCMTA + dollar amounts

## v6 — 2026-05-03

Final coverage release. Consolidates every fix accumulated during v5's iterative debug session into one clean release. **Highlights are a complete parser-visual-map**: every label that anchors a value is PALE, every value is STRONG, and the rect-overlap dedup is now smart enough to never drop a legitimate label or value due to font-margin pixel jitter between adjacent lines.

**Final highlight counts**

| Sample | Highlights | Notes |
|---|---:|---|
| `Carry_Freight_19_Dec_2018` | **920** | 30 pages, 1 conviction, 9 CVSA inspections, 0 collisions, 0 violations |
| `Carrier_Profile_30_Sept_2019` | **2096** | 39 pages, 13 convictions, 25 CVSA inspections, 1 collision, 1 violation |

**What's covered (per Part)**

| Part | Section title | Banner totals | Column headers | Detail field labels | Group/Category labels | Values |
|---|:-:|:-:|:-:|:-:|:-:|:-:|
| Cover + TOC      | ✅ | — | — | — | — | ✅ |
| Part 1 Carrier   | ✅ | — | — | NSC#/MVID/Safety/Op/Effective/Expiry/Fleet labels | — | NSC, MVID, address, R-Factor, contributions, fleet sizes |
| Part 2 Conviction | ✅ | DOCUMENTS:/CONVICTIONS:/ACTIVE POINTS: | DATE/DOCUMENT/JUR/VEHICLE/DRIVER NAME + TIME/JURISDICTION/DATE ENTERED | ISSUING AGENCY:/LOCATION:/DRIVER:/VEHICLE:/COMMODITY:/CCMTA CODE:/CONV DATE:/DOCKET NO: | 16 CCMTA groups | dates, docs, drivers, plates, offences, points |
| Part 3 CVSA       | ✅ | PASSED:/REQUIRED ATTENTION:/OUT OF SERVICE: | DATE/CVSA/DOCUMENT/JUR/AGENCY/PLATE/LEVEL/RESULT/TIME/DATE ENTERED + TYPE/VIN/YEAR/MAKE/CVSA DECAL # | AGENCY:/LOCATION:/DRIVER:/BY VEHICLE/OOS+REQ defect headers | 19+1 defect categories (with wording variants) | dates, docs, drivers, plates, VINs, defects |
| Part 4 Collision  | ✅ | totals labels | DATE/DOCUMENT/JUR/PLATE/STATUS/SEVERITY | ASSESSMENT:/DRIVER:/LOCATION:/VEHICLE: | Property Damage/Injury/Fatal | dates, docs, drivers, plates, VIN |
| Part 5 Violation  | ✅ | DOCUMENTS:/OFFENCES: | DATE/DOCUMENT/JUR/VEHICLE/DRIVER NAME + TIME/JURISDICTION/DATE ENTERED | ISSUING AGENCY:/LOCATION:/DRIVER:/VEHICLE:/COMMODITY:/ACT/SECTION:/CCMTA CODE:/TEXT: | 16 CCMTA groups | dates, docs, drivers, plates, offences |
| Part 6 Monitoring | ✅ | — | full monitoring summary + details columns | Stage 1-4 thresholds, Industry block | — | scores, percentages, thresholds, industry averages |

**Inherited fixes (now stable)**
- Rect-overlap dedup with **intersection-over-min-area > 0.5** — adjacent-line text rects no longer collide due to font ascender/descender margins. Restored ~500 highlights that v5.0's naive bounding-box test was wrongly dropping.
- Comprehensive label coverage — all PDF column headers, banner totals labels, detail-block field labels, end markers, and section titles included. (v4's frontend-only scope reverted.)
- `VEHICLE:` (with colon) added to PART2_LABELS and PART5_LABELS — `VEHICLE` (no colon) was the column header; both are needed.
- `CVSA_DEFECT_LABELS` extended with form-revision variants for category 9 (`Part II Section 8 OOSC only` vs `Part II Section 9 only`), category 17 (`(Buses)` vs column-clipped `(Buse`), and category 20 (`Driver's Seat (Missing)`, added 2019+).
- Part 3 column headers fully populated: `DATE`, `TIME`, `DOCUMENT`, `JUR`, `LEVEL`, `RESULT`, `DATE ENTERED`, `TYPE`, `VIN`, `YEAR`, `MAKE`, `CVSA DECAL #`, plus analysis-table words `TOTAL`, `PERCENT`, `OF TOTAL`.
- Schema extensions for newer-format 2021+ Carrier Profile: `carrier.{certificateNumber, certificateEffectiveDate, certificateExpiryDate}` and the new `pull.adminPenalty*` arrays + matching `$defs`.
- 2019 sample populated with `carrier.certificateEffectiveDate: "2018-03-23"` and `carrier.certificateExpiryDate: "Continuous"`.

**Validator**: `2/2 OK`. Both samples are schema-valid and pass all 14 cross-field math checks.

## v5 — 2026-05-03 (mid-version iterative)
