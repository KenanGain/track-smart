...
def check_math(data: dict, name: str) -> list[str]:
    ...
    # 4. Sum convictionAnalysis[].count == totalConvictions
    ...
    if soos != totals["oos"]:
        msgs.append(f"Sum cvsa OOS ({soos}) != cvsaDefectTotals.oos ({totals['oos']})")
    if sreq != totals["req"]:
        msgs.append(f"Sum cvsa REQ ({sreq}) != cvsaDefectTotals.req ({totals['req']})")
    if stot != totals["total"]:
        msgs.append(f"Sum cvsa total ({stot}) != cvsaDefectTotals.total ({totals['total']})")
    ...
    # 7. Sum collisionTotals[].count == collisionSummary.length == collisionDetails.length == totalCollisions
    cs = sum(r["count"] for r in pull["collisionTotals"])
    if cs != pull["totalCollisions"]:
        msgs.append(f"Sum collisionTotals.count ({cs}) != totalCollisions ({pull['totalCollisions']})")
    ...
    # 8. Sum collisionTotals[].points == Sum collisionDetails[].activePoints
    ...
    if cp_totals != cp_details:
        msgs.append(f"Sum collisionTotals.points ({cp_totals}) != Sum collisionDetails.activePoints ({cp_details})")
    ...
    if so != pull["totalViolations"]:
        msgs.append(f"Sum violationDetails offences ({so}) != totalViolations ({pull['totalViolations']})")
