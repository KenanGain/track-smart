...
# ── §5 page-16 tables: Summary + Defect Breakdown (index 15) ─────────
DEFECT_LABELS = [
    ("40", "Driver"),
    ("41", "Lighting Devices"),
    ("42", "Windhield, Wipers"),       # PDF actually misspells "Windshield"
    ("43", "Steering"),
    ("44", "Braking System"),
    ("45", "Brake Adjustment"),
    ("46", "Suspension"),
    ("47", "Tires, Wheels, Rims"),
    ("48", "Fuel System"),
    ("49", "Exhaust System"),
    ("50", "Coupling Devices"),
    ("51", "Body, Frame"),
...
def parse_cvsa_summary(d: fitz.Document) -> list[dict]:
    """§5 Summary table — 4 rows × 4 numeric columns (count / OOS / Fail / Pass)."""
    lines = [ln.rstrip() for ln in d[15].get_text().splitlines()]
    out: list[dict] = []
    for itype in SUMMARY_INSPECTION_TYPES:
        try:
            i = lines.index(itype)
        except ValueError:
            continue
        try:
            ...
            oos = int(lines[i + 2].strip())
            fail = int(lines[i + 3].strip())
            pass_ = int(lines[i + 4].strip())
        except (ValueError, IndexError):
            continue
        out.append({"inspectionType": itype, "count": count, "oos": oos, "fail": fail, "pass": pass_})
    return out


def parse_defect_breakdown(d: fitz.Document) -> list[dict]:
    """§5 CVSA Defect Type table. Each row's numeric values follow the label.
    PDF prints 0 / 4 / 6 numeric cells:
      0 → empty defect row
      4 → OOS + %OOS + Total + %Total (Fail columns blank)
      6 → OOS + %OOS + Fail + %Fail + Total + %Total"""
    lines = [ln.rstrip() for ln in d[15].get_text().splitlines()]
    out: list[dict] = []
    for code, label in DEFECT_LABELS:
        composite = f"{code} - {label}"
        i = next((k for k, ln in enumerate(lines) if ln.strip() == composite), None)
        if i is None:
            continue
        nums: list[float] = []
        j = i + 1
        while j < len(lines):
            n = _to_num(lines[j].strip())
            if n is None:
                break
            nums.append(n)
            j += 1
        oos = oos_pct = fail = fail_pct = total = total_pct = None
        if len(nums) == 6:
            oos, oos_pct, fail, fail_pct, total, total_pct = nums
        elif len(nums) == 4:
            oos, oos_pct, total, total_pct = nums
        out.append({
            "code": code, "label": label,
            "oos": int(oos) if oos is not None else None,
            "oosPct": float(oos_pct) if oos_pct is not None else None,
            "fail": int(fail) if fail is not None else None,
            "failPct": float(fail_pct) if fail_pct is not None else None,
            "totalDefects": int(total) if total is not None else None,
            "totalPct": float(total_pct) if total_pct is not None else None,
        })
    return out
...
def main():
    ...
    # CVSA — page-16 tables (Summary + Defect Breakdown)
    new_summary = parse_cvsa_summary(d)
    print(f"cvsa.summary: {len(pull['cvsa']['summary'])} -> {len(new_summary)}")
    pull["cvsa"]["summary"] = new_summary

    new_defects = parse_defect_breakdown(d)
    print(f"cvsa.defectBreakdown: {len(pull['cvsa']['defectBreakdown'])} -> {len(new_defects)}")
    pull["cvsa"]["defectBreakdown"] = new_defects

    # CVSA list (per-inspection, pages 17-33)
