...
def annotate(src_pdf: Path, dst: Path) -> None:
    ...
    for i in range(doc.page_count):
        ...
        if "Section 5 - CVSA Inspection" in text or "CVSA Inspection Results" in text:
            ...
            # CVSA Inspection Levels reference table — level numbers (1–7, C, R)
            # are 1-char tokens that would cause false positives if added globally.
            # Highlight them only on the page that prints the levels reference,
            # by finding each description and drawing a PALE rect to the left.
            if "CVSA Inspection Levels" in text:
                level_pairs = [
                    ("1", "Full Inspection (Vehicle & Driver)"),
                    ("2", "Walk Around Inspection"),
                    ("3", "Driver Only Inspection"),
                    ("4", "Special Inspection"),
                    ("5", "Vehicle Only Inspection"),
                    ("6", "Enhanced - Radioactive"),
                    ("7", "Other Provincial"),
                    ("C", "School Bus - Complete"),
                    ("R", "School Bus - Re-Inspection"),
                ]
                for _, desc in level_pairs:
                    rects = page.search_for(desc)
                    if not rects:
                        continue
                    # search_for() returns matches in document order. The Levels
                    # reference table comes before the Summary table, so the
                    # first hit is always the Levels-table row. (Some
                    # descriptions like "Vehicle Only Inspection" also match
                    # the pluralised Summary row label — those later hits are
                    # ignored here so we only expand the Levels-table cell.)
                    r = rects[0]
                    level_rect = fitz.Rect(r.x0 - 28, r.y0 - 1.0,
                                            r.x0 - 3, r.y1 + 1.0)
                    page.draw_rect(level_rect,
                                    color=STRONG["PURPLE"], fill=PALE["PURPLE"],
                                    fill_opacity=0.22, width=0.9, overlay=True)
                    hl.counts["PURPLE"] = hl.counts.get("PURPLE", 0) + 1
