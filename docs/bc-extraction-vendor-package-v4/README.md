# BC CVSE Carrier Profile Report PDF Extraction — Vendor Package (v4)
...
**Status:** v4 — field-coverage completeness pass. The annotator's promise is now: **every leaf value in `extracted.json` is added to the value collectors and produces a STRONG highlight wherever the printed text matches.** v3 added the §4/§6 summary tables; v4 closes the remaining leaf-value gaps that were silently being skipped — `pull.asOfDate`, `complianceReview.scores[].category`/`events`, `interventions[].description`, pending-contravention `cls`/`status`, accident `time`/`jur`/`dlJur`/`plateJur`/`vehDesc`/`charges`/`pts`, CVSA unit `kind`. **5032** highlights (up from 4981). Schema unchanged from v3; validator stays green. Frontend code unchanged.
