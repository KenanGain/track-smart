# BC CVSE Carrier Profile Report PDF Extraction — Vendor Package (v3)
...
**Status:** v3 — data-completeness release on top of v2's annotator fixes. The §4 Contraventions Summary cells now match the printed PDF (12 CCMTA groups + Totals row) and a new `pull.accidentSummary` array models the §6 Summary tile (Fatality / Injury / Property / Total Accidents counts × At Fault / Fault Unknown / Not at Fault / Active Points). The annotator highlights every cell of both tables. Schema is extended; validator stays green. Frontend code unchanged.
