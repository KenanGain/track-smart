# BC CVSE Carrier Profile Report PDF Extraction — Vendor Package (v5)
...
**Status:** v5 — data-truthing release. v4 made the annotator collect every leaf value in `extracted.json`. v5 fixes the JSON itself so every section reflects what the PDF actually prints: **CVSA list now has 90 inspections (was 12 frontend-mock subset)**, **accidents now has 5 (was 11 fabricated rows)**, **CVIP is empty (PDF says "No CVIP")**, **§4.4 pending-carrier is empty (PDF says "no contraventions")**. A new `scripts/rebuild_arrays_from_pdf.py` reference parser walks the PDF text and rebuilds those four arrays — vendors can use it as a column-layout guide. **5767** highlights (up from 5242). Schema gains the `Fatality` accident type; validator stays green. Frontend code unchanged.
