# Extraction map — 03072022_Ontario.pdf

Page-by-page guide for this specific PDF (17 pages). Read this side-by-side with `annotated.pdf` while you build your parser.
...
|---|---|---|---|
| **CVOR / RIN #** | `carrier.cvorNumber` (`030-720-022`) | Carrier Profile header | 🟢 |
| **Client Name** | `carrier.legalName` (`NORTH ROUTES TRANSPORT LTD.`) | Carrier Profile header | 🟢 |
| **Operating As** | `carrier.operatingAs` (`NORTH ROUTES`) | Carrier Profile DBA badge | 🟢 |
...
| **Search Date and Time** | `source.searchDate`, `pull.reportDate` (`2024-07-25`) | Pull row + Pull Snapshot title | 🟡 |
...
| **# of Collisions with points** | `pull.collWithPoints` (`6`) | Collision Details "With Points" | 🔵 |
| **# of Collisions not pointed** | `pull.collWithoutPoints` (`16`) | Collision Details "Without Points" | 🔵 |
...
|---|---|---|---|
| **Overall Violation Rate** | `pull.rating` | **24.16%** | CVOR Performance overall rating + Pull-by-Pull "Rating" |
| Collision row → **% Overall Contribution** | `pull.colContrib` | 2.52 | Pull-by-Pull "Col%" |
| Conviction row → **% Overall Contribution** | `pull.conContrib` | 5.06 | Pull-by-Pull "Con%" |
| Inspection row → **% Overall Contribution** | `pull.insContrib` | 16.57 | Pull-by-Pull "Ins%" |
| Collision **% of set Threshold** | `pull.colPctOfThresh` | 6.3 | CVOR Performance Collision tile |
| Conviction **% of set Threshold** | `pull.conPctOfThresh` | 12.65 | CVOR Performance Conviction tile |
| Inspection **% of set Threshold** | `pull.insPctOfThresh` | 82.85 | CVOR Performance Inspections tile |
...
| **Total Conviction Points** | `pull.convictionPoints` | 71 | Conviction Details "Total Points" |
...
| PDF label | JSON path | Value | UI |
|---|---|---|---|
| **Overall Out of Service %** | `pull.oosOverall` | 25.33% | Pull-by-Pull "OOS Ov%" |
| **Vehicle Out of Service %** | `pull.oosVehicle` | 26.67% | Pull-by-Pull "OOS Veh%" |
| **Driver Out of Service %** | `pull.oosDriver` | 4.00% | Pull-by-Pull "OOS Drv%" |

### # of Inspections by Level table

| Level | count | OOS | UI |
|---|---:|---:|---|
| Level 1 | 59 | 19 | Pull Snapshot → CVOR Rating Comparison "Level 1" |
| Level 2 | 44 | 13 | "Level 2" |
| Level 3 | 37 | 4 | "Level 3" |
| Level 4 | 5 | 1 | "Level 4" |
| Level 5 | 3 | 0 | "Level 5" |

> Sum of `count` (148) MUST equal `inspectionStats.cvsaInspections` (148). ✓
...
|---|---:|---:|
| 1 (most recent 8 mo) | 8 | 6 |
| 2 (middle 8 mo)      | 7 | 5 |
| 3 (earliest 8 mo)    | 7 | 3 |
| **Sum**              | **22** | **14** |

→ `pull.collisionBreakdown[0..2].{events,points}`. Sum must equal `collisionEvents` (22) and `totalCollisionPoints` (14). ✓
...
→ `pull.convictionBreakdown[0..2].{events,points}`. Sum must equal `convictionEvents` (26) and `convictionPoints` (71). ✓

### Inspection Threshold Calculation (drives Inspection Statistics mini section)

| Field | Value | JSON path |
|---|---:|---|
| # of CVSA inspections conducted | 148 | `pull.inspectionStats.cvsaInspections` |
| # of Vehicles inspected | 222 | `pull.inspectionStats.vehiclesInspected` |
| # of Drivers inspected | 148 | `pull.inspectionStats.driversInspected` |
...
| # of Set inspection threshold points | 57.19 | `pull.inspectionStats.setThreshold` |
| % of Set Threshold | 82.85% | (= `pull.insPctOfThresh`) |

> Formula: `0.6875 × 2 + 46 = 47.38 ≈ totalInspectionPoints (47.38)` ✓

---

## Pages 4 – 15 — Intervention & Event Details (🟣🔴🟠)

This is the longest section: each row is one event. **Inspection rows** (🟣 PURPLE), **Collision rows** (🔴 RED), **Conviction rows** (🟠 ORANGE).

For this PDF: **22 collisions + 26 convictions + 148 inspections = 196 total events**.
...
> - `events.filter(type==='inspection').length === inspectionStats.cvsaInspections` (148)
> - `events.filter(type==='collision').length === collisionEvents` (22)
> - `events.filter(type==='conviction').length === convictionEvents` (26)

---

## Pages 16+ — Travel Kilometric Information (🟤)
