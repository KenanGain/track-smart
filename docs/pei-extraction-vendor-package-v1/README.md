# PEI Carrier Abstract Report PDF Extraction — Vendor Package (v1)
...
**Status:** v1 — initial release. Two canonical examples (`Carrier_Profile_18_Nov_2020` and `Carrier_Profile_09_Mar_2023`). Both PDFs are scanned image-only documents — vendors must provide an OCR pipeline. Schema matches the existing TrackSmart frontend (`src/pages/inspections/NscPeiPerformanceCard.tsx` + `NscPeiPerformanceHistory.tsx`).
...
│   │   ├── extracted.json
│   │   ├── annotated.pdf       (7-color region overlay drawn at form coordinates)
│   │   ├── lists/*.csv
│   │   └── README.md
...
└── scripts/                 ← helper scripts
    ...
    ├── annotate_pdf.py      (raw PDF → annotated.pdf with rect overlay)
