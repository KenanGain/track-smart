...
PART4_LABELS = [
    "Summary", "Group Description", "and Equivalency Codes",
    ...
    "Mechanical Defects (0600-0699)", "Cargo Securement (0700-0799)",
    "Dangerous Goods (0800-0899)", "Vehicle Insurance (0900-0999)",
    "DL#/Jur:", "Class:", "Status:",
...
PART5_LABELS = [
    ...
    "Power Unit", "Semi-Trailer", "Trailer 1", "Trailer 2", "Bus",
]
...
class Highlighter:
    ...
    def add(self, source: str, color, kind: str, color_name: str, opacity: float = 0.40) -> None:
        if not source:
            return
        s = str(source).strip()
        if len(s) < 3:
    ...
    def flush(self) -> dict[str, int]:
        ...
        for _, _, color, opacity, rect, color_name in self.candidates:
...
def collect_values(data: dict) -> dict:
    ...
    add(G, dem.get("mailingAddress"))
    if dem.get("numberOfLicensedVehicles") is not None:
        add(G, str(dem["numberOfLicensedVehicles"]))
    ...
    for t in pull.get("thresholds", []):
        add(B, t["contraventions"]); add(B, t["cvsa"]); add(B, t["accidents"]); add(B, t["total"])
    ...
    for r in pull.get("contraventionSummary", []):
        if r["violations"]: add(O, str(r["violations"]))
        if r.get("violationsPct"): add(O, f"{r['violationsPct']:.2f}%")
        if r["activePoints"]: add(O, str(r["activePoints"]))
