# BC CVSE Carrier Profile Report PDF Extraction — Vendor Package (v1)
...
**Status:** v1 — initial release. One canonical sample (`Inertia_Carrier_2025-04-17`). BC PDFs are **text-extractable** so the data-driven `search_for` annotator works the same way it does for AB and CVOR (no OCR pipeline needed). Schema matches the existing TrackSmart frontend (`src/pages/inspections/NscBcCarrierProfile.tsx` + `NscBcPullByPull.tsx` + `NscBcPerformanceHistory.tsx`).
