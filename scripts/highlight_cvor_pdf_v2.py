...
V2_RAW    = REPO_ROOT / "docs" / "cvor-extraction-vendor-package-v2" / "raw-pdfs"
V2_OUT    = REPO_ROOT / "docs" / "cvor-extraction-vendor-package-v2" / "highlighted-pdfs"

DEFAULT_BATCH: list[Path] = sorted(V2_RAW.glob("*.pdf")) if V2_RAW.exists() else []
...
BLUE_FIELDS: list[FieldRule] = [
    ...
    # Page 2 — Inspections by Level + OOS by Level (drives levelStats L1-5)
    ...
    # Page 2 — NEW v2: collisionDetails severity breakdown.
    # Lenient (strict=False) so we still match if the OCR glues the label and
    # value together (e.g. "Fatal Collisions", "Property Damage Only").
    ...
    # Page 2/3 — NEW v2: convictionDetails category + points-status.
    # All marked non-strict because the MTO form embeds these as table-row
    # headers like "Driver Convictions", "Vehicle Convictions", etc.
    ...
    # Bare-word fallbacks. PURPLE/RED rules with longer phrases (e.g.
    # "Driver Action", "Vehicle Plate") still match first via bucket priority,
    # so these only catch standalone "Driver"/"Vehicle"/"Load"/"Other" cells.
    ...
    # Page 2/3 — Collision & Conviction Breakdown by Kilometre Rate (3 sub-period rows)
    ...
    # Page 3 — Inspection Threshold Calculation (drives inspectionStats)
...
def main(argv: list[str]) -> int:
    if argv:
        ...
        out_dir = V2_OUT
        if not sources:
            print(f"ERROR: no PDFs found in {V2_RAW}", file=sys.stderr)
            return 1
