...
Defaults:
    Reads:  docs/cvor-extraction-vendor-package-v5/raw-pdfs/*.pdf
    Writes: docs/cvor-extraction-vendor-package-v5/highlighted-pdfs/<name>.annotated.pdf
"""
...
V5_RAW    = REPO_ROOT / "docs" / "cvor-extraction-vendor-package-v5" / "raw-pdfs"
V5_OUT    = REPO_ROOT / "docs" / "cvor-extraction-vendor-package-v5" / "highlighted-pdfs"

DEFAULT_BATCH: list[Path] = sorted(V5_RAW.glob("*.pdf")) if V5_RAW.exists() else []
...
PURPLE_FIELDS: list[FieldRule] = [
    ...
    # FIX: was "Category*" in v4, never matched the data-row "Category"
    # token because lbl_norm kept the asterisk. v5 also adds a non-asterisk
    # variant explicitly.
...
def main(argv: list[str]) -> int:
    if argv:
        ...
        out_dir = V5_OUT
        if not sources:
            print(f"ERROR: no PDFs found in {V5_RAW}", file=sys.stderr)
            return 1
