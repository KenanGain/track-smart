...
export type Remark = { id: string; text: string; at: string; author: string };
...
export type Applicant = {
    ...
    formId: string; carrier: string; template: string;
...
type Addr = { street: string; city: string; state: string; zip: string; dates?: string };
type Lic = { number: string; cls: string; authority: string; exp: string; cdl: string; endorsements: string };
type Emp = { employer: string; position: string; dates: string; reason: string };
type Edu = { school: string; study: string; dates: string };
type Rec = { label: string; detail: string };
type Profile = {
    phone: string; dob: string; idNumber: string; twic?: string;
...
function build(a: Pick<Applicant, "firstName" | "lastName" | "email" | "formId" | "position">, p: Profile): SubSection[] {
    ...
    return [
        { title: "Applicant Information", groups: [{ fields: [
            ...
            { label: "TWIC Card Number", value: p.twic ?? "—" },
        ] }] },
        ...
        { title: "License Details", groups: p.licenses.length ? p.licenses.map((l, i) => ({
            ...
            fields: [
                ...
                { label: "Licensing Authority", value: l.authority },
...
const SEED: Applicant[] = [
    seedApplicant(
        { id: "a1", firstName: "Jane", lastName: "Doe", email: "jane.doe@example.com", formId: "us", carrier: "Acme Logistics", template: "Complete Driver Hiring (Default)", status: "under-review", stepsDone: 12, stepsTotal: 12, invitedAt: "Jun 2, 2026", updatedAt: NOW - 6 * H, position: "OTR Driver", phone: "(312) 555-0148" },
        {
            phone: "(312) 555-0148", dob: "03/14/1988", idNumber: "***-**-4471", twic: "8821940",
            addresses: [
            ...
            licenses: [
                { number: "D1234-5678-90", cls: "Class A", authority: "Illinois", exp: "03-04-2027", cdl: "Yes", endorsements: "HazMat, Tanker" },
                { number: "D9988-1122-33", cls: "Class B", authority: "Indiana", exp: "07-15-2024", cdl: "Yes", endorsements: "None" },
            ],
            employment: [
                { employer: "Roadrunner Freight", position: "OTR Driver", dates: "01-2021 - 03-2024", reason: "Career advancement" },
                { employer: "Prairie Haul Co.", position: "Regional Driver", dates: "05-2018 - 12-2020", reason: "Relocated" },
            ],
    ...
    seedApplicant(
        { id: "a2", firstName: "Marcus", lastName: "Reed", email: "marcus.reed@example.com", formId: "local", carrier: "Acme Logistics", template: "Fast-Track Hiring", status: "submitted", stepsDone: 12, stepsTotal: 12, invitedAt: "Jun 4, 2026", updatedAt: NOW - 2 * H, position: "Local Driver", phone: "(773) 555-0192" },
        {
            ...
            licenses: [{ number: "R8821-1140-22", cls: "Class B", authority: "Illinois", exp: "11-30-2028", cdl: "Yes", endorsements: "None" }],
            employment: [{ employer: "Midwest Distribution", position: "Local Delivery Driver", dates: "06-2019 - 05-2024", reason: "Seeking local route" }],
    ...
    seedApplicant(
        { id: "a3", firstName: "Priya", lastName: "Patel", email: "priya.patel@example.com", formId: "canada", carrier: "Northwind Transport", template: "Complete Driver Hiring (Default)", status: "in-progress", stepsDone: 5, stepsTotal: 12, invitedAt: "Jun 5, 2026", updatedAt: NOW - 35 * MIN, phone: "(416) 555-0173" },
        {
    ...
    seedApplicant(
        { id: "a4", firstName: "Tom", lastName: "Bell", email: "tom.bell@example.com", formId: "us", carrier: "Acme Logistics", template: "Complete Driver Hiring (Default)", status: "waiting", stepsDone: 0, stepsTotal: 12, invitedAt: "Jun 7, 2026", updatedAt: NOW - 1 * H },
    ),
    seedApplicant(
        { id: "a5", firstName: "Sofia", lastName: "Nguyen", email: "sofia.nguyen@example.com", formId: "cross-border", carrier: "Blue Ridge Freight", template: "Complete Driver Hiring (Default)", status: "approved", stepsDone: 12, stepsTotal: 12, invitedAt: "May 28, 2026", updatedAt: NOW - 3 * D, position: "Cross-Border Driver", phone: "(206) 555-0120" },
        {
            phone: "(206) 555-0120", dob: "05/30/1985", idNumber: "***-**-3320", twic: "7741203",
            addresses: [
    ...
    seedApplicant(
        { id: "a6", firstName: "Derek", lastName: "Olsen", email: "derek.olsen@example.com", formId: "us-owner-operator", carrier: "Northwind Transport", template: "Owner-Operator Hiring", status: "in-progress", stepsDone: 8, stepsTotal: 12, invitedAt: "Jun 6, 2026", updatedAt: NOW - 5 * H, position: "Owner-Operator", phone: "(503) 555-0166" },
        {
    ...
    seedApplicant(
        { id: "a7", firstName: "Hassan", lastName: "Ali", email: "hassan.ali@example.com", formId: "local", carrier: "Acme Logistics", template: "Fast-Track Hiring", status: "rejected", stepsDone: 9, stepsTotal: 12, invitedAt: "May 30, 2026", updatedAt: NOW - 2 * D, phone: "(312) 555-0188" },
        {
