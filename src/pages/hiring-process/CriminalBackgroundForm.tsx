...
export function CriminalBackgroundForm({ onBack, embedded, startPreview }: { onBack: () => void; embedded?: boolean; startPreview?: boolean }) {
    ...
    const sections: DocSection[] = [
        { title: "Background Check", groups: [{ rows: [
    ...
    return (
        <FormScaffold
            ...
            docTitle="Criminal Background Check" docSubtitle={status || undefined} badge={passFail || undefined} sections={sections} branding={branding} fileName="criminal-background-check.pdf"
            intro={<>Record the criminal background check — request &amp; completion dates, the status and result, the pass / fail determination, and upload the report. Add comments and complete the review checklist below.</>}
        >
            <div>
            ...
            <div>
                <SectionTitle>Report Document</SectionTitle>
            ...
            <ReviewSignOff heading="I have reviewed the background check above." value={signoff} onChange={setSignoff} />
        </FormScaffold>
    );
}
