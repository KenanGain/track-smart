...
export function UserEditModal({ user, currentUser, onClose, onSave }: Props) {
    ...
    useEffect(() => {
        ...
        const initialSps = user.serviceProfileIds && user.serviceProfileIds.length > 0
            ? user.serviceProfileIds
            : (user.serviceProfileId ? [user.serviceProfileId] : []);
        setServiceProfileIds(initialSps);
        setApplyToAllServiceProfiles(false);
