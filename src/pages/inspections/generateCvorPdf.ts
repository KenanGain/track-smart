...
// Mirror of generateFmcsaPdf for the CVOR report.

import { createRoot, type Root } from "react-dom/client";
import { createElement } from "react";
import html2canvas from "html2canvas";
import jsPDF from "jspdf";
import { CvorPdfReport, type CvorPdfReportProps } from "./CvorPdfReport";

export interface GenerateCvorPdfOptions extends CvorPdfReportProps {
    fileName?: string;
}
...
export async function generateCvorPdf(options: GenerateCvorPdfOptions): Promise<void> {
    ...
    try {
        ...
        root.render(createElement(CvorPdfReport, reportProps));
...
function defaultFileName(props: CvorPdfReportProps): string {
    const carrier = (props.carrierProfile.name || "Carrier").replace(/\s+/g, "_");
    const cvor = props.carrierProfile.cvor ?? "";
    const d = (props.reportDate || new Date()).toISOString().slice(0, 10);
    return `CVOR-Report_${carrier}${cvor ? "_CVOR" + cvor : ""}_${d}.pdf`;
}
